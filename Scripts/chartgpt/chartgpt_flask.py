...
# )
# def store_data(contents, filename):
#     print("store_data callback triggered")  # Indicates callback is triggered
#     if contents is not None:
#         print(f"Filename: {filename}")  # Prints the filename of the uploaded file
#         result = parse_contents(contents, filename)
#         print(f"Result of parse_contents: {result}")  # Prints the result after parsing
#         if isinstance(result, pd.DataFrame):
#             return result.to_json(orient="split")
#         else:
#             print(result)  # Log the error message or result
#             return no_update
...
# #         result = parse_contents(contents, filename)
# #         if isinstance(result, pd.DataFrame):
# #             # Only convert to JSON if the result is a DataFrame
# #             return result.to_json(orient="split")
# #         else:
# #             # If result is a string (an error message), log the message or handle it as needed
# #             print(result)  # Log the error message for debugging purposes
# #             # Here you could also update the front-end to display the error
# #             return no_update  # Return no_update to indicate no new data should be stored
# #     return no_update  # Return no_update if contents is None
...
#     default_response = (html.Div(), html.Div(), {}, html.Div())

#     # Check if dataset is None, which happens on initial load or if no file is uploaded
#     if dataset is None:
#         # This is expected on the initial load or when no file is uploaded yet
#         return default_response

#     print("Dataset provided, processing...")
#     try:
#         # Check if dataset is a JSON string
#         if dataset.startswith('{') and dataset.endswith('}'):
#             print("Dataset format is JSON, converting to DataFrame...")
#             df = pd.read_json(io.StringIO(dataset), orient='split')
#         else:
#             print("Dataset format is not JSON, assuming file path...")
#             df = pd.read_csv(dataset)

#         # Create the table preview component
#         table_preview = dag.AgGrid(
#             id="data-preview",
#             rowData=df.to_dict("records"),
#             style={"height": "275px"},
...
#         # Construct and return the UI components
#         return (
#             table_preview,
#             table_preview,
#             {
...
#     # No need for a separate 'else' block here as all cases are handled above
...
# def predict(api_key, df_json, prompt):
#     import numpy as np
#     # Convert the JSON to a DataFrame
#     df = pd.read_json(df_json, orient="split")
#   # Assuming the first column is 'time' or has a name containing 'time'
#     time_column = [col for col in df.columns if 'time' or 'date' in col.lower()]
    
#     if not time_column:
#         print("No time column found")
#         return df  # or handle this as needed
#     else:
#         time_column = time_column[0]  # Take the first column that matches

#     # Convert the 'time' column to datetime
#     df[time_column] = pd.to_datetime(df[time_column])

#     # Exclude non-numeric columns for operations like correlation, except the 'time' column
#     numeric_columns = df.select_dtypes(include=[np.number]).columns.tolist()
#     numeric_df = df[[time_column] + numeric_columns]


#     # Exclude non-numeric columns for operations like correlation
#     # numeric_df = df.select_dtypes(include=[np.number])

#     # If you need to convert dates to a numeric format, you could do something like this:
#     # df['date'] = pd.to_datetime(df['date_column']).values.astype(np.int64)  # Convert to timestamp

#     # Now you can create an instance of ChartGPT with the numeric DataFrame
#     chart = cg.Chart(numeric_df, api_key=api_key)

#     # Proceed with plotting
#     fig = chart.plot(prompt, return_fig=True)

#     # Generate the output to display
#     output = show_graph_card(graph=dcc.Graph(figure=fig), code=chart.last_run_code)
